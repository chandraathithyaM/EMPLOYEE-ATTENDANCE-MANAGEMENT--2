...
import { playUltrasonic } from '../utils/ultrasonic';
...
const AttendanceOtp = () => {
    ...
    useEffect(() => {
        ...
        } else if (timeLeft === 0 && (otp || qrCodeData || ultrasonicToken)) {
            // Signal expired
    ...
    }, [timeLeft, otp, qrCodeData, ultrasonicToken]);

    useEffect(() => {
        return () => stopUltrasonicSignal();
    }, []);
    ...
    return (
        <div className="attendance-section">
            ...
            <div className="verification-mode-selector" style={{ 
                display: 'flex', 
                gap: '10px', 
                marginBottom: '20px',
                padding: '10px',
                ...
                <span style={{ alignSelf: 'center', opacity: 0.7, marginRight: 10 }}>Verification:</span>
                {['LOCATION', 'ULTRASONIC', 'WIFI'].map(m => (
                    <button
                        key={m}
                        className={`btn ${verificationMode === m ? 'btn-primary' : 'btn-outline'}`}
                ...
                <div className="ultrasonic-section">
                    ...
                    {isPlayingUltrasonic ? (
                        <div className="ultrasonic-visualizer">
                            ...
                            <button className="btn btn-outline btn-sm" style={{ marginTop: 10 }} onClick={stopUltrasonicSignal}>
                                <FaStop style={{ marginRight: 6 }} /> Stop Signal
                            </button>
                        </div>
                    ) : (
