...
import {
    ...
    FaKey, FaQrcode, FaMapMarkerAlt, FaWifi, FaVolumeUp, FaMicrophone, FaStop
} from 'react-icons/fa';
import { getProfile, getMyAttendance, getMyLeaves, markMyAttendance } from '../services/api';
import { listenForUltrasonic } from '../utils/ultrasonic';
...
const EmployeeDashboard = () => {
    ...
    const [verificationMode, setVerificationMode] = useState('LOCATION'); // 'LOCATION' | 'ULTRASONIC' | 'WIFI'
    ...
    try {
        ...
        // ✅ Safely ensure arrays — handles any API response shape
        setAttendance(Array.isArray(attendanceRes.data) ? attendanceRes.data : attendanceRes.data?.content ?? []);
        setLeaves(Array.isArray(leavesRes.data) ? leavesRes.data : leavesRes.data?.content ?? []);

    } catch (err) {
        setError('Failed to load data');
        setAttendance([]); // ✅ prevent crash even on error
        setLeaves([]);
    } finally {
        setLoading(false);
    }
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                setTimeout(() => {
                    if (audioCtxRef.current) {
                        stopListening();
                        setAttendanceMsg({ type: 'info', text: 'Ultrasonic listening timed out after 30s. Please try again.' });
                        resolve(null);
                    }
                }, 30000);
